...
///
fn cluster_serialize<S>(cluster: &Option<String>, ser: S) -> Result<S::Ok, S::Error>
where
    S: Serializer,
{
...
fn base64_serialize<S>(bytes: &Option<Vec<u8>>, ser: S) -> Result<S::Ok, S::Error>
where
    S: Serializer,
{
...
impl NewComputation {
    pub fn insert_computation(&self, pool: &DbPool) -> Computation {
        ...
        computations.order(id.desc()).first(&db_conn).unwrap()
    }
}
